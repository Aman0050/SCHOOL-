...
export const CollectionHealth: React.FC<any> = ({ stats }) => {
  const getHealthColor = (score: string) => {
    switch (score) {
      ...
      case 'Good': return 'text-blue-500 bg-blue-50 dark:bg-blue-500/10 border-blue-200 dark:border-blue-500/20';
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-2 gap-4 w-full mt-6">
          <div className="text-center p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
            <div className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Defaulters</div>
            <div className="text-xl font-bold text-slate-800 dark:text-white mt-1">{stats?.defaultersCount || 0}</div>
          </div>
          <div className="text-center p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
            <div className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Fines Due</div>
            <div className="text-xl font-bold text-slate-800 dark:text-white mt-1">₹{stats?.outstandingFines?.toLocaleString() || 0}</div>
          </div>
        </div>
      </div>
    </div>
